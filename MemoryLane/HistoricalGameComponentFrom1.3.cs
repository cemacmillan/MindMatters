...
public class MindMattersGameComponent : GameComponent
{
    ...
    private int initializationDelay = 150; // Delays processing to ensure safe initialization
    ...
    public override void GameComponentTick()
    {
        ...
        if (!isInitialized)
        {
            if (currentTick >= initializationDelay)
            {
                isInitialized = true;
                Log.Message("[MindMatters] Initialization complete. Starting processing.");
            }
            return;
        }

        // Log and register DynamicNeeds once, after initialization
        if (debugNeedsRegistry)
        {
            MMToolkit.DebugLog("GameComponentTick: NeedDefs");
            DynamicNeedFactory.DebugLogRegisteredDynamicNeeds();
            debugNeedsRegistry = false;
        }

        // Schedule regular processing
        if (currentTick % 900000 == 0) // Every game hour
        {
        ...
        if (currentTick % 300 == 0) // Every 5 seconds
        {
            ...
            needsMgr.ProcessNeedsWithDelay(DynamicNeedCategory.Primal);
        }

        if (currentTick % 600 == 0) // Every 10 seconds
        {
            needsMgr.ProcessNeedsWithDelay(DynamicNeedCategory.Secondary);
        }

        if (currentTick % 1200 == 0) // Every 20 seconds
        {
            needsMgr.ProcessAllNeeds(immediate: false, specificDelay: 1200);
        }

        if (currentTick % 60000 == 0) // Every day
        {
