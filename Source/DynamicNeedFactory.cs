...
public static class DynamicNeedFactory
{
    ...
    public static DynamicNeed CreateNeedInstance(Type needType, Pawn pawn)
    {
        ...
        try
        {
            NeedDef? needDef = DynamicNeedsRegistry.GetNeedDef(needType);
            if (needDef == null)
                return null;
    ...
    public static void ApplyEventTriggeredNeeds(Pawn pawn, string eventType)
    {
        switch (eventType)
        {
            ...
            default:
                MMToolkit.GripeOnce($"[MindMatters] Unknown event type '{eventType}' for needs creation.");
                break;
        }
    }
    ...
    {
        DynamicNeed? newNeed = CreateNeedInstance(needType, pawn);
        if (newNeed != null)
        {
            // pawn.needs.AddOrRemoveNeed(newNeed.def, true);
            MMToolkit.DebugLog($"[MindMatters] Added need '{newNeed.def.defName}' to {pawn.LabelShort}.");
        }
    }
}